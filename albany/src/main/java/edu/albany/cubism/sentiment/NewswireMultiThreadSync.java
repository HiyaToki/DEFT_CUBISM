...
public class NewswireMultiThreadSync {
    ...
    public String createThreads(String docID) throws InterruptedException, IOException {
        ...
        for (int i = 0; i < 8; i++) {
            threadCollection.add(new Thread(new RunnableDemo("t" + (i + 1), docID, "", this)));
...
class RunnableDemo implements Runnable {
    ...
    private NewswireMultiThreadSync runObj; // An object from the MultiThradSync method we just came from

    // Initialize the variables for the Runnable thread
    public RunnableDemo(String thread_name, String docID, String finalPathName, NewswireMultiThreadSync runObj) {
    ...
    public void run() {
        try {
            ...
            this.finalPathName = new NewswireAnnotations().lookupThreads(this.docID, this.thread_name, this.finalPathName);
