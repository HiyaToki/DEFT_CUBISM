 ...
 * @author tejasrao
 */
public class CubismSentimentAssessment {

    private static CubismSentimentAssessment setUp = new CubismSentimentAssessment();
    ...
    public String Testing() throws IOException {
        ...
        String kbp_data_path = Settings.getValue("kbp_data_path");
        String kbp_sentiment_path = Settings.getValue("kbp_sentiment_path");
        String curr_path = System.getProperty("user.dir");
        if (curr_path.endsWith("test-classes") ||
                curr_path.endsWith("test-classes/")) {
            kbp_data_path = curr_path+"/us/ihmc/cubism/sociolinguistics/Data/";
            kbp_sentiment_path = curr_path+"/us/ihmc/cubism/sociolinguistics/Data/";
        }
        String inputFile = kbp_data_path+"/2014Answer";
        System.out.println("inputFile: " + inputFile);
        String outputFile = kbp_sentiment_path+"/base_sorted_merged.txt";
        String testFile = kbp_sentiment_path+"/base_sorted_formatted1.txt";
        CustomFileReader answerOutput = new CustomFileReader(kbp_data_path+"/2014evaluation/annotation/all_forum_assessment.txt");
                    ...
                    for (int i = 0; i < assessItems.size(); i++) {
                        ...
                        if (assessItems.get(i).docID.contentEquals(newsDocID) && assessItems.get(i).queryID.contentEquals(newsQueryID)) {
                            ...
                            for (String docs : indDocs) {
                                ...
                                int expLower = lowerOffsetAssess - 75;
                                int expUpper = upperOffsetAssess + 75;
...
//                                        System.out.println("FALSE:\t" + currLine);
        ...
        result.append("\nRecall:\t\t\t\t" + (df.format(recall) + "%")).append("\n");
        System.out.println("Precision:\t\t\t" + (df.format(precision) + "%"));
        result.append("Precision:\t\t\t" + (df.format(precision) + "%") + "\n");
        System.out.println("F-Measure:\t\t\t" + (df.format(fmeasure)));
        result.append("F-Measure:\t\t\t" + (df.format(fmeasure) + "%"));
