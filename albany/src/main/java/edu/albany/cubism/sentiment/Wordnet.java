...
//                JWNL.initialize(new FileInputStream(curr_path+"/us/ihmc/cubism/sociolinguistics/conf/file_properties3.0.xml"));
                System.out.println("curr_path: " + curr_path + "/us/ihmc/cubism/sociolinguistics/conf/file_properties3.0.xml");
                String path = curr_path + "/us/ihmc/cubism/sociolinguistics/conf/file_properties3.0.xml";
                this.resetFileProperties(path, curr_path);
                JWNL.initialize(new FileInputStream(curr_path + "/us/ihmc/cubism/sociolinguistics/conf/file_properties3.0.xml"));
            } else {
                JWNL.initialize(new FileInputStream("./src/main/resources/us/albany/cubism/conf/file_properties3.0.xml"));
            }
...
//        System.out.println(this.pu_);
    ...
    public void resetFileProperties(String path, String curr_path) {
        ...
        try {
            ...
            if (curr_path.indexOf("test-classes") != -1) {
                String new_path = curr_path + "/../../src/main/ext/WordNet-3.0/dict";
//                System.out.println(new_path);
                content = content.replace("/export/u10/jobs/4.10_target_focused_sentiment_extraction_tests/workspace/ihmc/src/main/ext/WordNet-3.0/dict", new_path);
                content = content.replace("../../", "/../../");
            } else {
                content = content.replace("/export/u10/jobs/4.10_target_focused_sentiment_extraction_tests/workspace/ihmc/src/main/ext/WordNet-3.0/dict", curr_path + "/src/main/ext/WordNet-3.0/dict");
            }
...
//            System.out.println(iw);
    ...
    public ArrayList getVerbFrame(String phrase, String pos) {
        ...
        for (Synset phr_syn : phr_syns) {
            ...
            Word[] rel_words = (Word[]) phr_syn.getWords();
            for (Word rel_word : rel_words) {
...
//    }
    ...
    public void printHypernymHierarchy(String word, String pos) {
        try {
            ArrayList list = new ArrayList();
            ArrayList<PointerTargetTree> ptr_tree_list = new ArrayList();
            POS _pos = null;
            if (pos.equals("noun")) {
                _pos = NOUN;
            } else if (pos.equals("verb")) {
            ...
            for (int i = 0; i < senses.size(); i++) {
                ...
                ptr_tree_list.add(pu_.getHypernymTree(sense));
            }
            PointerTargetTreeNode node;
            for (PointerTargetTree tree : ptr_tree_list) {
    ...
    public void printHyponymHierarchy(String word, String pos) {
        try {
            ArrayList<PointerTargetTree> ptr_tree_list = new ArrayList();
            POS _pos = null;
            if (pos.equals("noun")) {
            ...
            for (int i = 0; i < senses.size(); i++) {
                ...
                ptr_tree_list.add(pu_.getHyponymTree(sense));
            }
            PointerTargetTreeNode node;
            for (PointerTargetTree tree : ptr_tree_list) {
                node = tree.getRootNode();
    ...
    private void printList(PointerTargetTreeNode node) {
        printListHelper(node, 0);
    }

    private void printListHelper(PointerTargetTreeNode node, int space_count) {
        System.out.print("\t");
        for (int i = 0; i < space_count; i++) {
            System.out.print(". ");
        }
        System.out.print("(" + (space_count + 1) + ")\t");
        trimmer(node.getSynset().toString());
        for (Object object_representation : node.getChildTreeList()) {
            PointerTargetTreeNode casted_node = (PointerTargetTreeNode) object_representation;
            printListHelper(casted_node, ++space_count);
        }
    }

    public ArrayList<Synset> getHypernymsAsSynsets(String word, String pos, int recursion_level) {
        ...
        try {
            ...
            for (PointerTargetTree tree : ptr_tree_list) {
                node = tree.getRootNode();
                synsets.addAll(getSynsets(node, recursion_level));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<Synset> getHyponymsAsSynsets(String word, String pos, int recursion_level) {
        ArrayList<Synset> synsets = new ArrayList();
        try {
            ArrayList<PointerTargetTree> ptr_tree_list = new ArrayList();
            POS _pos = null;
    ...
    private ArrayList<Synset> getSynsets(PointerTargetTreeNode node, int recursion_level) {
        ArrayList<Synset> synsets = new ArrayList();
        getSynsetsHelper(node, 0, recursion_level, synsets);
        return synsets;
    }

    private void getSynsetsHelper(PointerTargetTreeNode node, int recursion_level, int specified_recursion_level, ArrayList<Synset> synsets) {
        Synset set = node.getSynset();
        if (!synsets.contains(set)) {
            synsets.add(set);
        }
        if (recursion_level >= specified_recursion_level) {
            return;
        }
        for (Object object_representation : node.getChildTreeList()) {
            PointerTargetTreeNode casted_node = (PointerTargetTreeNode) object_representation;
            getSynsetsHelper(casted_node, ++recursion_level, specified_recursion_level, synsets);
        }
    }
...
//            e.printStackTrace();
    ...
    public int getSenseCount(String word, String pos) {
        ...
        return this.getSenses(word, pos1).size();
    }
...
//        }
    ...
    /*
     public HashMap<Synset, HashSet<Synset>> getHyperSynsetsFront_Nlvl_Mses_actCtrl(String word, String pos, Integer N, Integer M, boolean doActCtrl) {
     HashMap<String, HashMap<Synset, HashSet<Synset>>> map = null;
     HashMap<Synset, HashSet<Synset>> result = new HashMap<Synset, HashSet<Synset>>();
     //close hyper use different map
     ...
     public HashSet<Synset> getHyperSynsetsOfSense_Nlvl_actCtrl(Synset sense, String pos, Integer N, boolean doActCtrl) {
     HashMap<Synset, HashSet<Synset>> map = null;
     //close hyper use different map
...
//        }
     ...
     */
    /*
     public HashMap<String, Synset> isHyper_2WordsFront_Nlvl_Mses_actCtrl(String s1, String s2, String pos, Integer N, Integer M, boolean doActCtrl) {
     ...
     */
    /*
     public Synset isHyper_ofOffset_Nlvl_Mses_actCtrl(long offset, String s2, String pos, Integer N, Integer M, boolean doActCtrl) {
     Synset sense1 = this.getSynsetAtOffset(pos, offset);
     ...
     public boolean isHyper_2Offsets_Nlvl_actCtrl(long offset1, long offset2, String pos, Integer N, boolean doActCtrl) {
...
//                System.out.println("---"+currSize);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("less baseform: " + w.getBaseForm("less", "adjective"));
//        
//        ArrayList<String> search_terms = new ArrayList() {
//            {
//                add("suffer");
//            }
//        };//new CustomFileReader().getFileAsArrayListOfString("/home/administrator/Downloads/input_file");
////        String pos = "noun";
//        String pos = "verb";
////        String pos = "verb";
//        Collections.sort(search_terms);
//        ArrayList<Synset> list;
//        for (String term : search_terms) {
//            System.out.println(term);
////            System.out.println(w.getSenses(term, POS.NOUN).size());
//            System.out.println("###################################");
//            list = w.getHyponymsAsSynsets(term, pos, 1);
//            for(Synset s : list){
//                System.out.println(s);
//                
//            }
//            for (Synset s: list) {
//                String word = s.getWord(0).getLemma();
////                System.out.println("get senses of " + word);
//                ArrayList word_syns = w.getSynonSynsetsFront_Mses_actCtrl(word, pos, 40, false);
//                int word_posi = word_syns.indexOf(s);
////                System.out.println("add(new ClusterEntry<String>(" + word + "\", -1, " + (word_posi+1) + ", " + s.getOffset() + ", " + word + ", " + word + "\"));"); //noun
////                System.out.println("add(new ClusterEntry<String>(\"" + word + "\", -1, " + (word_posi+1) + ", " + s.getOffset() + ", \"" + word + "\", \"" + term + "\"));"); //noun
//                System.out.println("en_actions_.add(new ClusterEntry<String>(\"" + word + "\", -1, " + (word_posi+1) + ", " + s.getOffset() + ", \"" + word + "\", \"" + term + "\"));"); //verbs
//            }
//            System.out.println();
//        }
    }

    public static void trimmer(String strLine) {
        String split[];
        strLine = strLine.substring(strLine.indexOf(":") + 1, strLine.lastIndexOf("]"));
        strLine = strLine.substring(strLine.indexOf(":") + 1, strLine.length()).replaceAll("[\\[\\]]", "").trim();
        strLine = strLine.replace("Words:", "@").replace("POS:", "@").replaceAll("--", "@");
        split = strLine.split("@");
